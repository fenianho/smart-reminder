...
class OpenAiService(private val config: AiConfig) : AiService {
    ...
    override suspend fun parseReminderText(text: String): Result<AiParseResult> {
        Log.d(TAG, "Starting to parse reminder text with OpenAI: $text")
        val startTime = System.currentTimeMillis()
        
        return withContext(Dispatchers.IO) {
            try {
                val prompt = createParsePrompt(text)
                Log.d(TAG, "Sending request to OpenAI API for parsing")
                val response = makeApiCall(prompt)
                val duration = System.currentTimeMillis() - startTime

                if (response.isSuccess) {
                    Log.d(TAG, "OpenAI API call successful. Duration: ${duration}ms")
                    val parsed = parseResponse(response.getOrThrow())
                    Log.d(TAG, "Successfully parsed response from OpenAI - Title: ${parsed.title}, " +
                            "Description: ${parsed.description}, Time: ${parsed.scheduledTime}, Repeat: ${parsed.repeatType}")
                    Result.success(parsed)
                } else {
                    Log.w(TAG, "OpenAI API call failed. Duration: ${duration}ms")
    ...
    override suspend fun suggestReminderTime(title: String, description: String?): Result<List<AiTimeSuggestion>> {
        Log.d(TAG, "Starting to suggest reminder time with OpenAI. Title: $title")
        val startTime = System.currentTimeMillis()
        
        return withContext(Dispatchers.IO) {
            try {
                val prompt = createTimeSuggestionPrompt(title, description ?: "")
                Log.d(TAG, "Sending request to OpenAI API for time suggestion")
                val response = makeApiCall(prompt)
                val duration = System.currentTimeMillis() - startTime
                ...
                if (response.isSuccess) {
                    ...
                    Log.d(TAG, "Successfully parsed time suggestions from OpenAI. Count: ${suggestions.size}")
    ...
    override suspend fun isAvailable(): Boolean {
        Log.d(TAG, "Checking OpenAI service availability")
        return withContext(Dispatchers.IO) {
            try {
                // 简单检查API密钥是否有效
                ...
                client.newCall(request).execute().use { response ->
                    ...
                    Log.d(TAG, "OpenAI availability check result: $isAvailable. Duration: ${duration}ms. " +
    ...
    private fun createRequestBody(prompt: String): okhttp3.RequestBody {
        Log.d(TAG, "Creating request body for OpenAI API")
    ...
    private fun parseResponse(responseBody: String): AiParseResult {
        try {
            Log.d(TAG, "Parsing OpenAI response")
            ...
            if (choices.length() > 0) {
                ...
                Log.d(TAG, "Successfully parsed result from OpenAI response - Title: $title, " +
    ...
    private fun parseTimeSuggestions(responseBody: String): List<AiTimeSuggestion> {
        try {
            Log.d(TAG, "Parsing time suggestions from OpenAI response")
            val json = JSONObject(responseBody)
            val choices = json.getJSONArray("choices")
            if (choices.length() > 0) {
                val message = choices.getJSONObject(0).getJSONObject("message")
